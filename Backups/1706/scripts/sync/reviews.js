...
async function syncReviews(page, ws, syncData) {
  ...
  const baseTimeout   = 30000;   // базовые 30 000 мс
  let   extraTimeout  = 0;

  try {
    // Если Press & Hold капча всё ещё видна — дадим +25 000 мс
    const captchaPresent = await page.$('#px-captcha');
    if (captchaPresent) {
      console.log('[syncReviews] → Обнаружена капча. Увеличиваем таймаут.');
      extraTimeout = 25000;
    }
  } catch (_) { /* игнор */ }
  // ------------------------------------------
  ...
  try {
      await waitForStateChange(() => reviewsData !== null, 2000, baseTimeout + extraTimeout);
  } catch (timeoutErr) {
