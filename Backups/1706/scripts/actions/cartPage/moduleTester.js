...
const WAIT_TIME_AFTER_NAVIGATION = 500; // Задержка (мс) после перехода по URL, для стабилизации страницы
...
const testedModule = require('./clearCartFast.js').clearCartFast


// const testedModule = require('../utils/reviewManager.js').reviewManager;
...
async function runTest() {
  ...
  try {
    ...
    // await testedModule(page, { promoCode: "MART30", referralCode: "DUA9670" });

    await testedModule(page, {
  verbose: true,   // логируем этапы
  sniff:   true    // хотим поймать cartcount + recommendations
});
