...
/**
 * Сравнивает два ФИО, считая совпадением любую перестановку частей.
 * Например: "Берикбаев Нуржан" и "Нуржан Берикбаев" будут считаться одинаковыми.
 */
function areNamesEquivalent(nameA, nameB) {
  if (!nameA || !nameB) return false;
  const arrA = nameA.trim().split(/\s+/).map(str => str.toLowerCase()).sort();
  const arrB = nameB.trim().split(/\s+/).map(str => str.toLowerCase()).sort();
...
async function fillCustomsInfo(page) {
  ...
  // Сравниваем отображаемое имя из блока Customs с именем из блока доставки (DeliveryAddress).
  try {
    ...
    if (areNamesEquivalent(customsName, deliveryName)) {
      console.log('[customsInfo] -> Данные в таможенной информации и адресной книге совпадают, пропускаем заполнение.');
      return;
    }
  } catch (error) {
    console.log('[customsInfo] -> Не удалось проверить соответствие таможенной информации:', error.message);
    // Если не удалось проверить, продолжаем попытку заполнения.
  }
  ...
  // 3) Получаем отображаемое имя из блока адресной книги (DeliveryAddress).
  let displayedName;
  try {
  ...
  // 4) Ищем запись пользователя в адресной книге по гибкому совпадению (без учёта регистра, частичному содержанию).
  ...
  // 8) Ждём, пока исчезнет спиннер загрузки. Если не исчез за 10 сек, перезагружаем страницу.
  const spinnerSelector = 'div.ConnectedLoading__SpinnerWrapper-sc-1245kmi-0.dFApI';
  try {
    ...
    console.log('[customsInfo] -> Спиннер не исчез за 10 сек, перезагружаем страницу...');
    await page.reload({ waitUntil: 'networkidle2' });
    try {
      await page.waitForSelector(spinnerSelector, { hidden: true, timeout: 10000 });
      console.log('[customsInfo] -> Спиннер исчез после перезагрузки, данные загружены.');
    } catch (err) {
      console.log('[customsInfo] -> После перезагрузки спиннер так и не исчез:', err.message);
    }
  }

  // 9) Проверяем данные через API (пример). Ждём ответ о таможне, если нужно.
  try {
    const customsResponse = await page.waitForResponse(
      response =>
