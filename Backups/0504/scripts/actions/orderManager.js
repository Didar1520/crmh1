...
function mergeParams(item) {
  return {
    ...
    setAdressPage: item.setAdressPage === true,
...
async function processAllOrders() {
  ...
  for (let i = 0; i < inputOrders.length; i++) {
    ...
    // Если есть cartLink => сначала addressManager + handleCart, потом синхронизации
    if (!isEmpty(finalParams.cartLink)) {
      console.log('[orderManager] -> Найден CartLink, запускаем addressManager.js перед handleCart().');
      try {
        // Примерно так: await addressManager(); (если нужно дождаться)
        addressManager(); // если addressManager ничего асинхронного не делает – вызов без await
      ...
      try {
        ...
        console.log('[orderManager] -> Пропускаем синхронизацию для этого элемента.');
        continue;
      }
      // Если дошли сюда, значит заказ прошёл без ошибок
      await handleSyncFlags(item, currentPage);

    } else {
      // Если cartLink нет => сразу выполняем sync-флаги
      await handleSyncFlags(item, currentPage);
    }
