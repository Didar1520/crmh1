...
async function cardVerification(page) {
  ...
  if (cardWarning) {
    ...
    if (!selectedCard) {
      ...
      try {
        // 1) Ждём появления элемента iframe с классом .js-iframe
        ...
        // 3) В контексте фрейма ждём поле #encryptedCardNumber
        await frame.waitForSelector('#encryptedCardNumber', { visible: true, timeout: 10000 });
        // Кликаем по инпуту, чтобы сфокусироваться на нём
        await frame.click('#encryptedCardNumber', { clickCount: 3 });

        // 4) Стираем текущее значение напрямую (если оно есть)
