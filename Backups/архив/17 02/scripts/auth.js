// scripts/auth.js
/**
 * ТУТ УБИРАЕМ setViewport (оно теперь в mainScript),
 * + оборачиваем чтение response.text() в try/catch + проверку content-type
 */
...
const config = require('../config'); // может понадобиться для timeouts
...
async function authorize(page, params, ws) {
  ...
  try {
    await page.goto('https://kz.iherb.com', {
      ...
      timeout: config.browserConfig.timeouts.pageWaitTime
    });
  ...
  try {
    await page.waitForSelector('#truste-consent-button', {
      visible: true,
      timeout: config.browserConfig.timeouts.cookieWaitTime
    });
  ...
  try {
    await page.waitForSelector(selectors.buttonGotoLogin, {
      visible: true,
      timeout: config.browserConfig.timeouts.defaultWaitTime
    });
    await humanMouseMovements(page, 1500);
  ...
  try {
    await page.waitForSelector('#px-captcha', {
      timeout: config.browserConfig.timeouts.captchaWaitTime
    });
