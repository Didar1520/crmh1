...
async function addNewAddress(page, addressData) {
  ...
  while (reloadAttempts < 3) {
    try {
      ...
      await page.type(selectors.fullNameInput, fullName, typingOptions);
      await page.type(selectors.addressLine1Input, addressData.Street), typingOptions;
      await page.type(selectors.cityInput, addressData.City), typingOptions;
      await page.type(selectors.regionInput, addressData.Region), typingOptions;

      // Приводим значение почтового индекса к строке
      const postString = addressData.Post ? addressData.Post.toString() : '';
      await page.type(selectors.postalCodeInput, postString, typingOptions);

      // Формируем строку телефона (например, добавляя +)
      const phoneString = addressData.Number ? `+${addressData.Number.toString()}` : '+';
      await page.type(selectors.phoneNumberInput, phoneString, typingOptions);
