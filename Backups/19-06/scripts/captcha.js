...
async function solvePressAndHoldCaptcha(page) {
  let reloadCount = 0;
// --- быстрая проверка: если контейнера уже нет, выходим мгновенно ---
const initHasCaptcha = await page.$('#px-captcha');
if (!initHasCaptcha) {
  console.log('[captcha.js] -> Контейнер #px-captcha не найден, считаем решённой.');
  return true;
}
// --------------------------------------------------------------------
...
/* ── конец отметки ── */
      ...
      if (ok) return true;
// если во время ожидания контейнер исчез — капча решена стороной сервера
const vanished = await page.$('#px-captcha').then(e => !e);
if (vanished) {
  console.log('[captcha.js] -> Контейнер исчез во время цикла ⇒ капча решена.');
  return true;
}
...
const stillExists = await page.$('#px-captcha');
if (!stillExists) {
  console.log('[captcha.js] -> Контейнер пропал перед перезагрузкой ⇒ капча решена.');
  return true;
}
