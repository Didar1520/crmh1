...
async function changeCurrency(page, currencyCode) {
  try {
    ...
    // 1. Если уже нужная валюта — ничего не делаем
    const currentVal = await page.$eval('.currency-select span', el => el.textContent.trim());
    if (currentVal.toUpperCase() === currencyCode.toUpperCase()) {
      console.log('[setCartParams] -> Валюта уже нужная, пропускаем');
      return;
    }

    // 2. Убеждаемся, что панель открыта
    const drawerOpen = await page.evaluate(() =>
      document.querySelector('.selected-country-wrapper[data-on="on"]') !== null
    );
    if (!drawerOpen) {
      await page.click('.selected-country-wrapper');
      await page.waitForSelector('.select-currency', { visible: true, timeout: 5000 });
    }

    // 3. Кликаем по вкладке «Currency» (нужно для новой разметки)
    const currencyTab = await page.$('.select-currency');
    if (currencyTab) {
      await currencyTab.click();
    }

    // 4. Ждём поле поиска (любой из двух вариантов)
    await page.waitForSelector(
      '.select-currency input[type="search"], .select-currency .search-input',
      { visible: true, timeout: 5000 }
    );

    // 5. Очищаем поле и печатаем код валюты
    const searchBox = await page.$('.select-currency input[type="search"]')
                   || await page.$('.select-currency .search-input');
    await searchBox.click({ clickCount: 3 });
    await searchBox.type(currencyCode, { delay: 40 });

    // 6. Выбираем найденный пункт (два возможных селектора)
    await page.waitForSelector(
      `div.item[data-val="${currencyCode}"], li.list-item[data-val="${currencyCode}"]`,
      { visible: true, timeout: 5000 }
    );
    await page.click(
      `div.item[data-val="${currencyCode}"], li.list-item[data-val="${currencyCode}"]`
