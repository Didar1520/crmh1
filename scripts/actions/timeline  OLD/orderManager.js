...
// ----- цветные коды ANSI -----
const GREEN = '\x1b[32m';   // зелёный
const BLUE  = '\x1b[34m';   // синий
const RESET = '\x1b[0m';    // вернуть обычный цвет
...
async function processAllOrders() {
  ...
  for (let i = 0; i < inputOrders.length; i++) {
           ...
           // --- шлём событие «шаг начат» ---    // Сливаем поля
...
if (!alreadyAuth) {
    ...
    // Если есть CartLink => запускаем handleCart, а потом синхронизацию
      ...
      if (!isEmpty(finalParams.cartLink)) {
        ...
        handleCart(currentPage, finalParams));
          ...
          if (res.type === 'completed') {if (res.type === 'completed') {
  const promo = finalParams.promoCode || finalParams.promo || '—';
  const line  = `корзина N${i + 1} [${finalParams.cartLink}] ${rec.orderNumber} [${promo}] ${rec.client}`;
  console.log(line);          // видно сразу после заказа
  // сохраним эти данные для финального отчёта
  ...
  // строки для клиента
  report += '\nДля клиента (скопировать и отправить):\n';
  processedOrders.forEach(p => {
    if (p.status === 'completed') {
      report += `  корзина N${p.idx} [${p.cartLink}] ${p.orderNumber} [${p.promo}] ${p.client}\n`;
    }
  });
